...
async def setbirthday(interaction: discord.Interaction, month: int, year: int = datetime.datetime.now().year):
	await interaction.response.defer()
	jpcalendar = calendar.LocaleTextCalendar(locale='ja_jp.UTF-8')
...
async def change():
	tips = [
		...
		"TIPS: 私の御主人様は ねんねこ㌨様( @nennneko5787 ) ですわ！(このTIPSも御主人様が考えたんですよ！)",
		"TIPS: 御主人様は中学1年生ですわ！(2024年から2年生ですわ！)",
		...
		for row in result:
			...
			if birthday is not None:
				...
				if now.month == month and now.day == day:
					...
					message = await client.get_channel(1219491827422330910).send(f"🎂今日は{client.get_guild(1208388325954560071).get_member(id).mention}さんの推し①{nm}の{t}誕生日です！おめでとう！🎉")
					try:
						if message.flags.crossposted != True:
							await message.publish()
					except:
